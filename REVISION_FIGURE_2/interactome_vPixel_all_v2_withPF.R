...
# Rscript interactome_vPixel_all_v2_withPF.R

buildTable <-  TRUE

outFolder <- "INTERACTOME_VPIXEL_ALL_V2_WITHPF"
...
hicds <- paste0("GSE118514_", ref_hicds, "_40kb")
...
revFig_dt$dataset <- file.path(revFig_dt$hicds, revFig_dt$exprds)
...
filt_dt <- revFig_dt[revFig_dt$purityCorr > purityCorrThresh,]
filt_dt <- filt_dt[filt_dt$hicds == hicds & filt_dt$exprds == exprds,]
stopifnot(nrow(filt_dt) > 0)
...
final_table_DT$chromo <- gsub("(.+)_TAD.+", "\\1", final_table_DT$region)
final_table_DT$newID <- paste0(final_table_DT$chromo, "_", final_table_DT$start, "_",final_table_DT$end)
newIDs <- setNames(final_table_DT$newID, final_table_DT$regionID)

final_table_DT <- final_table_DT[final_table_DT$hicds == hicds & final_table_DT$exprds == exprds,]
...
###########################
### ADD HERE PURITY FILTER !!!!!!¨
###########################¨

filt_dt$chromo <- gsub("(.+)_TAD.+", "\\1", filt_dt$region)
stopifnot(filt_dt$region %in% final_table_DT$region)
stopifnot(filt_dt$regionID %in% names(newIDs))
filt_dt$newIDs <- newIDs[paste0(filt_dt$regionID)]
stopifnot(!duplicated(filt_dt$newIDs))

plot_dt$chr <- paste0("chr", gsub(" ", "", gsub("(.+):(.+):(.+):(.+)", "\\1", plot_dt$tad_name)))
plot_dt$start <- paste0("", gsub(" ", "", gsub("(.+):(.+):(.+):(.+)", "\\2", plot_dt$tad_name)))
plot_dt$end <- paste0("", gsub(" ", "", gsub("(.+):(.+):(.+):(.+)", "\\3", plot_dt$tad_name)))
plot_dt$newIDs <- paste0(plot_dt$chr, "_", plot_dt$start, "_", plot_dt$end)

outFile <- file.path(outFolder, paste0("filt_dt.Rdata"))
save(filt_dt, file=outFile,version=2)
stopifnot(filt_dt$newIDs %in% plot_dt$newIDs)

cat(paste0("nrow(plot_dt) == ", nrow(plot_dt), "\n"))
plot_dt <- plot_dt[plot_dt$newIDs %in% filt_dt$newIDs ,]
cat(paste0("nrow(plot_dt) == ", nrow(plot_dt), "\n"))

################################################################################################## 
################################################# prep data

tmp <- plot_dt[,c("tad_name", "tad_dir", "newIDs")]
tmp <- unique(tmp)
stopifnot(!duplicated(tmp$tad_name))

plot_dt$tad_dir_short <- plot_dt$tad_dir
plot_dt$tad_dir_short[grepl("notsignif",plot_dt$tad_dir_short )] <- "not signif." 


all_dirs_short <- tmp$tad_dir
...
save(plot_dt, file=file.path(outFolder, paste0(ref_hicds, "_fig5_density_pvals.Rdata")), version=2)
