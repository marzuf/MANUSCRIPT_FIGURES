...
source(file.path(runFolder, "annotated_TADs.R")); stopifnot(exists("annotated_tads"))
...
cptmt_dt <- get(load(file.path(runFolder, "REVISION_CHANGES_CPTMTLABELS_V3_NOTPF/plot_dt.Rdata")))
...
if(buildTable){
  ...
  inFile <- file.path(runFolder,"TAD_MATCHING_ACROSS_HICDS_NOPERMUT", "all_matching_dt.Rdata")
 ...
 matching_data <- foreach(mypair = all_pairs) %dopar% {
    ...
    all_norm_files <- list.files(file.path(runFolder, hicds_norm, "FINAL_DOMAINS_WITH_SCORES"), pattern="final_domains_with_scores.txt$", full.names = TRUE)
    ...
    norm_assigned_region_file <- file.path(runFolder, hicds_norm, "genes2tad", "all_assigned_regions.txt")
    ...
    all_tumor_files <- list.files(file.path(runFolder, hicds_tumor, "FINAL_DOMAINS_WITH_SCORES"), pattern="final_domains_with_scores.txt$", full.names = TRUE)
    # aggregate the rank values
    ...
    tumor_assigned_region_file <- file.path(runFolder, hicds_tumor, "genes2tad", "all_assigned_regions.txt")
