...
outFolder <- "CMP_GO_ALL_PARTIAL_FILTER_CPE"
...
discardTADs_aran <- get(load(file.path(runFolder, 
                              "TAD_MATCHING_SIGNIF_ACROSS_HICDS_ALLMATCH_v2_PURITYFILTER", "CPE", "log10", 
...
out_disc_dt <- merge(conservRegions_all_dt[,c("conserved_region","nDS", "intersect_genes_symbol")],aggAran_conservRegions_dt, by="conserved_region", all=TRUE )
...
head(out_disc_dt, 10)
out_disc_dt <- out_disc_dt[order(out_disc_dt$nDS, out_disc_dt$ratioDiscAran,decreasing = TRUE),]
head(out_disc_dt, 10)


outFile <- file.path(outFolder, "ratioDiscTADsInConservedRegions_aran.png")
png(outFile, width=600, height=400)
plot(density(out_disc_dt$ratioDiscAran), main="Ratio of disc. TADs in the conserved regions", xlab="ratio of. disc. TADs", cex.lab=1.2, cex.axis=1.2,cex.main=1.2)
mtext(side=3, text=paste0(nrow(out_disc_dt), " conserved regions - Aran CPE data"),cex=1.2)
legend("topright", legend=paste0(sum(out_disc_dt$ratioDiscAran >0.8), "/", nrow(out_disc_dt), " > 0.8"), bty="n")
foo <- dev.off()
