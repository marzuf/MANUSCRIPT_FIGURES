...
# Rscript go_signif_across_hicds_v2_withThresh_purityFilter_final.R
...
outFolder <- file.path("GO_SIGNIF_ACROSS_HICDS_v2_WITHTHRESH_PURITFILTER_FINAL", conservThresh, data_cmpType, purity_ds, pm, transfExpr)
...
if(! plotOnly) {
  ...
  for(var_plot in barplot_vars) {
    ...
    subTit <- paste0("(conserved in >= ", conservThresh, "/", nDS,  " datasets; purity filter: ", purity_plot_name, ")")
