...
# Rscript go_signif_across_hicds_v2_purityFilter_final.R
# Rscript go_signif_across_hicds_v2_purityFilter_final.R norm_vs_tumor
# Rscript go_signif_across_hicds_v2_purityFilter_final.R subtypes
# Rscript go_signif_across_hicds_v2_purityFilter_final.R wt_vs_mut

script_name <- "go_signif_across_hicds_v2_purityFilter_final.R"
...
outFolder <- file.path("GO_SIGNIF_ACROSS_HICDS_v2_PURITYFILTER_FINAL", data_cmpType, purity_ds, pm, transfExpr)
...
inFolder <- file.path(runFolder, "TAD_MATCHING_SIGNIF_ACROSS_HICDS_ALLMATCH_v2_PURITYFILTER_FINAL", data_cmpType, purity_ds, pm, transfExpr)
...
keptTADs <- get(load(file.path(inFolder, "signifTADs_to_keep.Rdata")))
head(keptTADs)
head(signif_tads)
if(purity_ds == "EPIC") stopifnot(signif_tads %in% keptTADs)

discTADs <- get(load(file.path(inFolder, "signifTADs_to_discard.Rdata")))
head(discTADs)
head(signif_tads)
stopifnot(!signif_tads %in% discTADs)
...
save(conserved_signif_enrich_resultDT, version=2, file=file.path(outFolder, "conserved_signif_enrich_resultDT_0.Rdata"))
    ...
    if(genes_signif_plotMax > 0) {
      for(var_plot in barplot_vars) {
        ...
        mtext(side=3, text=paste0("(# datasets = ", nDS, "; purity filter: ", purity_plot_name, ")"), font=3)
        foo <- dev.off()
...
save(conserved_signif_enrich_resultDT, version=2, file=file.path(outFolder, "conserved_signif_enrich_resultDT_2.Rdata"))
  ...
  for(var_plot in barplot_vars) {
    ...
    subTit <- paste0("(# datasets = ", nDS, "; purity filter: ", purity_plot_name, ")")
