...
outFolder <- file.path("FCC_WAVE_PLOT_NOABS_PURITYFILTER_FINAL_PF")
...
# Rscript fcc_wave_plot_noabs_purityFilter_final_PF.R


script0_name <- "0_prepGeneData"
script1_name <- "1_runGeneDE"
script8_name <- "8cOnlyFCCpF_runAllDown"  # HERE: LOAD PF 
script_name <- "fcc_wave_plot_noabs_purityFilter_final_PF.R"
...
all_fcc_dt <- foreach(hicds = all_obs_hicds, .combine='rbind') %dopar% {
    ...
    hicds_dt <- foreach(exprds = all_obs_exprds[[paste0(hicds)]], .combine='rbind') %do% {
      ...
      # if(! (hicds == "GSE105381_HepG2_40kb" & exprds == "TCGAlihc_norm_lihc")) return(NULL)
      ...
      rdFile <- paste0(pipOutFold, "/", script8_name, "/", curr_ratio_type, "_permDT.Rdata") # update PF 19.08.20 -> only 53
      if(!file.exists(rdFile)) return(NULL)
      ...
      cat(paste0("length(qt95Permut_cumsum)\t=\t", length(qt95Permut_cumsum), "\n"))
      cat(paste0("length(obs_cumsum)\t=\t", length(obs_cumsum), "\n"))
      cat(paste0("length(purityFtd_obs_cumsum)\t=\t", length(purityFtd_obs_cumsum), "\n"))
      
      stopifnot( length(purityFtd_obs_cumsum) == length(qt95Permut_cumsum) )
      
      
		par(mar = par()$mar + c(0,1,0,0))
      
      plot(#obs_cumsum ~ x_val,
        purityFtd_obs_cumsum ~ purityFtd_x_val,
           main= my_main,
           type = "l",
           pch = 16, cex = 0.7,
           xlab= my_xlab, 
      ...
      auc_obs <- auc(x = purityFtd_x_val, y = purityFtd_obs_cumsum)
      auc_permut <- auc(x = purityFtd_x_val, y = meanPermut_cumsum)
      auc_permutQt <- auc(x = purityFtd_x_val, y = qt95Permut_cumsum)
