...
all_dt_2=all_dt
save(all_dt_2, file="all_dt_2.Rdata",version=2)

all_dt$focusGenes <- all_dt$missedLowFCsameDir & 
                     (all_dt$TAD_rD >= rD_highThresh | all_dt$TAD_rD <= rD_lowThresh)

nFocus_dt <- aggregate(focusGenes ~ hicds+exprds, data=all_dt, FUN=sum)

focus_dt <- all_dt[all_dt$focusGenes,]

focus_dt$dataset <- file.path(focus_dt$hicds, focus_dt$exprds )

nRec_dt <- aggregate(dataset  ~ entrezID, data=focus_dt, FUN=function(x)length(unique(x)))
dsRec_dt <- aggregate(dataset  ~ entrezID, data=focus_dt, FUN=function(x)paste0(unique(x), collapse=","))
...
nMissedByTAD_dt <- aggregate(missedLowFCsameDir ~ hicds + exprds + rD_range + region, FUN=sum, data=focus_dt)
...
out_dt <- min2_nMissedByTAD_focus_dt
