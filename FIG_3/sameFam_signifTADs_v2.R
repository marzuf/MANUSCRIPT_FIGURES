...
# Rscript sameFam_signifTADs_v2.R

script_name <- "sameFam_signifTADs_v2.R"
...
outFolder <- "SAMEFAM_SIGNIFTADS_V2"
    ...
    exprds_dt <- foreach(exprds=all_obs_exprds[[paste0(hicds)]], .combine='rbind') %dopar% {
      ...
      kept_dt$nbrUniqueFams_lab <- ifelse(kept_dt$nbrUniqueFams == 1, "1 unique fam.", 
                                      ifelse(kept_dt$nbrUniqueFams == 2, "2 unique fam.", 
                                            ifelse(kept_dt$nbrUniqueFams > 2, ">= 3 unique fam.", NA)))
      ...
      agg_dt$nbrUniqueFams_lab <- factor(agg_dt$nbrUniqueFams_lab, levels=rev(c("1 unique fam.", "2 unique fam.", ">= 3 unique fam.")))
      ...
      if(hicds=="ENCSR489OCU_NCI-H460_40kb" & exprds=="TCGAluad_norm_luad") {
        ...
        p1 <- ggbarplot(data=agg_dt, y="nbr", x="tad_signif",
                        ...
                        subtitle=subTit) + labs(fill="") + scale_fill_nejm()
