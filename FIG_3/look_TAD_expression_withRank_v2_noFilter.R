...
startTime <- Sys.time()
cat(paste0("> Rscript look_TAD_expression_withRank_v2_noFilter.R\n"))

script_name <- "look_TAD_expression_withRank_v2_noFilter.R"
...
outFolder <- file.path("LOOK_TAD_EXPRESSION_WITH_RANK_V2_NOFILTER")
...
# CHANGED
geneList <- get(load(file.path(pipFolder, hicds, exprds, "0_prepGeneDataCheckRm", "pipeline_geneList_noCpmFilter.Rdata")))
stopifnot(geneList %in% g2t_dt$entrezID)
...
toplot_dt$value_log10 <- toplot_dt$value 
