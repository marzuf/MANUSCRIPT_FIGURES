...
# Rscript meanFC_meanCorr_dotSizeGG_v2.R ENCSR489OCU_NCI-H460_40kb TCGAlusc_norm_lusc chr11_TAD390 chr10_TAD268
# Rscript meanFC_meanCorr_dotSizeGG_v2.R ENCSR489OCU_NCI-H460_40kb TCGAluad_norm_luad chr11_TAD390 chr10_TAD268
# Rscript meanFC_meanCorr_dotSizeGG_v2.R ENCSR489OCU_NCI-H460_40kb TCGAluad_mutKRAS_mutEGFR chr10_TAD16 chr17_TAD162
# Rscript meanFC_meanCorr_dotSizeGG_v2.R ENCSR489OCU_NCI-H460_40kb TCGAluad_nonsmoker_smoker chr10_TAD16 chr17_TAD162
# 
...
outFolder <- "MEANFC_MEANCORR_DOTSIZEGG_V2"
...
basel_pal_transp <- get(load(file.path(outFolder, "basel_pal_transp.Rdata")))
# basel_pal <- piratepal(palette = "basel")
# save(basel_pal, file=file.path(outFolder, "basel_pal.Rdata"), version=2)
basel_pal <- get(load(file.path(outFolder, "basel_pal.Rdata")))
...
xmen_pal_transp <- get(load(file.path(outFolder, "xmen_pal_transp.Rdata")))

tad_verySignif_col <-  as.character(basel_pal[2])
tad_signif_col <- as.character(basel_pal_transp[2])

ratioConcord_col <- as.character(basel_pal[1])
ratioNotConcord_col <- as.character(basel_pal_transp[1])

ratioConcord_col <- "darkblue"
ratioNotConcord_col <- "cornflowerblue"


pval_cols <- c( "<= 0.01" = tad_verySignif_col,
                "<= 0.05" = tad_signif_col,
                "> 0.05" = as.character(xmen_pal_transp[5]))

ratioDown_cols <- c("<= 0.2 | >= 0.8" = ratioConcord_col,
                    "> 0.2 & < 0.8" = ratioNotConcord_col,
                    "not signif." = as.character(pval_cols[paste0("> 0.05")]))

ex_DT$pval_cat <- ifelse(ex_DT$adjPvalComb <= 0.01, "<= 0.01",
                               ifelse(ex_DT$adjPvalComb <= 0.05, "<= 0.05", "> 0.05"))
stopifnot(!is.na(ex_DT$pval_cat))

ex_DT$ratioDown_cat <- ifelse(ex_DT$ratioDown <= 0.2 | ex_DT$ratioDown >=0.8, "<= 0.2 | >= 0.8",
                          ifelse(ex_DT$ratioDown > 0.2 & ex_DT$ratioDown <0.8, "> 0.2 & < 0.8", NA))
stopifnot(!is.na(ex_DT$ratioDown_cat))

ex_DT$ratioDown_cat[ex_DT$pval_cat == "> 0.05"] <- "not signif."
stopifnot(!is.na(ex_DT$ratioDown_cat))

stopifnot(!is.na(ex_DT))

fc_corr_p <- ggplot(ex_DT, aes(x=meanLogFC, y = meanCorr, fill = pval_cat )) +
  ggtitle(plotTit, subtitle = subTit)+
  scale_fill_manual(values=pval_cols,
                    labels=setNames(names(pval_cols), pval_cols))+
  ...
  geom_point(data=ex_DT[ex_DT$ratioDown_cat=="not signif.",],
             shape=21,
             aes(size = adjPvalComb_log10_col, color = ratioDown_cat), stroke=1) +
  geom_point(data=ex_DT[ex_DT$ratioDown_cat!="not signif.",],
             shape=21,
             aes(size = adjPvalComb_log10_col, color = ratioDown_cat), stroke=1)+
...
#   }
# }
# ex_DT_rD_dt <- merge(ex_DT, all_dt, by=c("hicds", "exprds", "region"), all.x=T, all.y=F)
# stopifnot(!is.na(ex_DT_rD_dt))
