...
# Rscript sameFam_signifTADs_v3_sameSize.R

script_name <- "sameFam_signifTADs_v3_sameSize.R"
...
outFolder <- "SAMEFAM_SIGNIFTADS_V3_SAMESIZE"
...
if(buildTable) {
  ...
  all_sameFam_signif_dt <- foreach(hicds=all_obs_hicds, .combine='rbind') %do% {
    exprds_dt <- foreach(exprds=all_obs_exprds[[paste0(hicds)]], .combine='rbind') %do% {
      ...
      signif_kept_dt <- kept_dt[kept_dt$tad_signif == "signif.",]
      notSignif_kept_dt <- kept_dt[kept_dt$tad_signif == "not signif.",]
      stopifnot(nrow(signif_kept_dt) + nrow(notSignif_kept_dt) ==  nrow(kept_dt))
      
      ### CHANGE HERE TO SUBSAMPLE
      
      all_TAD_sizes <- unique(signif_kept_dt$nbrGenes)
      
      # for each TAD size, subsample as many random as observed
      allSamp_notSignif_kept_dt <- foreach(i = 1:nRandom, .combine='rbind') %dopar% {
        
        subSamp_notSignif_kept_dt <- foreach(tad_size = all_TAD_sizes, .combine='rbind') %do% {
          
          tmp_signif_dt <- signif_kept_dt[signif_kept_dt$nbrGenes == tad_size,]
          stopifnot(nrow(tmp_signif_dt) > 0)
          nSignif <- nrow(tmp_signif_dt)
          
          tmp_notSignif_dt <- notSignif_kept_dt[notSignif_kept_dt$nbrGenes == tad_size,]
          stopifnot(nrow(tmp_notSignif_dt) >= nSignif)
          
          
          sample_rows <- sample(x=1:nrow(tmp_notSignif_dt), size=nSignif)
          
          out_dt <- tmp_notSignif_dt[sample_rows,]
          stopifnot(nrow(out_dt) == nrow(tmp_signif_dt))
          out_dt
        }  
        stopifnot(nrow(subSamp_notSignif_kept_dt) == nrow(signif_kept_dt))
        stopifnot(sum(subSamp_notSignif_kept_dt$nbrGenes) == sum(signif_kept_dt$nbrGenes))
        subSamp_notSignif_kept_dt
      }
      stopifnot(nrow(allSamp_notSignif_kept_dt) == nRandom * nrow(signif_kept_dt))
      
      withSamp_kept_dt <- rbind(allSamp_notSignif_kept_dt, signif_kept_dt)
...
subTit <- paste0(names(nSignif_count), " : ", as.numeric(nSignif_count), collapse = "; ")
subTit <- paste0("TADs with ratioAnnot >=  ",ratioAnnotFilter, " - ",  subTit, " - sameSize ", nRandom, " subsamp." )
